...
export default function NavigationLink({ href, title, onClick, icon: Icon, isActive }: NavigationLinkProps) {
  return (
    ...
    >
      {Icon && <Icon className={`w-4 h-4 shrink-0 transition-transform duration-300 ${isActive ? "text-[var(--primary)]" : "group-hover:scale-110 group-hover:-rotate-5 group-hover:text-[var(--primary)]"}`} />}
      <span className="relative z-[1]">{title}</span>
      <span className={`absolute inset-0 -z-0 bg-gradient-to-br from-[var(--primary)] to-[var(--chart-1)] transition-opacity duration-300 ${isActive ? "opacity-10" : "opacity-0 group-hover:opacity-10"}`} aria-hidden />
    </Link>
  );
}
