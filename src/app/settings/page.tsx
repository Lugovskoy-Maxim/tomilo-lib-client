"use client";
import { Settings, Palette, Languages, Bell, Eye, Monitor } from "lucide-react";
          ...
          <div className="space-y-2 mb-2">
            ...
            <div className="bg-[var(--card)] rounded-xl border border-[var(--border)] p-2">
              ...
              {/* <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div>
                    <span className="text-sm font-medium text-[var(--foreground)]">Режим чтения</span>
                    <p className="text-xs text-[var(--muted-foreground)]">Предпочтительный режим</p>
                  </div>
                  <select className="px-3 py-1.5 bg-[var(--background)] border border-[var(--border)] rounded-lg text-sm">
                    <option>Страница за страницей</option>
                    <option>Прокрутка</option>
                    <option>Двойная страница</option>
                  </select>
                </div>
                <div className="flex items-center justify-between">
                  <div>
                    <span className="text-sm font-medium text-[var(--foreground)]">Автосохранение</span>
                    <p className="text-xs text-[var(--muted-foreground)]">Сохранять прогресс чтения</p>
                  </div>
                  <div className="relative inline-block w-10 h-5">
                    <input type="checkbox" id="auto-save" defaultChecked className="sr-only peer" />
                    <label
                      htmlFor="auto-save"
                      className="block w-full h-full bg-[var(--muted)] border border-[var(--border)] rounded-full cursor-pointer peer-checked:bg-[var(--primary)] peer-checked:border-[var(--primary)] transition-colors"
                    >
