...
function getLastReadInfo(entry: ReadingHistoryEntry): {
  ...
  const titleChapters = isPopulated
    ? (titleData!.chapters as TitleChapterInfo[] | undefined)
    : undefined;
...
export default function ContinueReading({ userProfile }: ContinueReadingProps) {
  ...
  const lastRead = useMemo(() => {
    ...
    const sorted = [...readingHistory].sort(
    ...
    return (
      <div className="profile-glass-card rounded-xl sm:rounded-2xl p-4 sm:p-5">
        ...
        <div className="text-center py-6 rounded-xl bg-[var(--secondary)]/30 border border-[var(--border)]/50">
          ...
          <p className="text-sm text-[var(--muted-foreground)]">Вы ещё ничего не читали</p>
          <Link
            href="/catalog"
