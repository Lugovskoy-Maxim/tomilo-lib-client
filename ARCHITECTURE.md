...
- **NEXT_SERVER_ACTIONS_ENCRYPTION_KEY** — в production **обязательно** задать стабильный ключ при сборке (см. `.env.example`). Без него после каждого деплоя у части пользователей (старый кеш или открытая вкладка) возникает ошибка «Failed to find Server Action» и сайт может не работать. Ключ: base64, длина AES (16/24/32 байта). Сгенерировать: `openssl rand -base64 32`.
