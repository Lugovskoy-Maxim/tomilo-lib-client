...
interface UnderratedCardProps {
  data: UnderratedCardData;
      ...
      <div className="relative overflow-hidden rounded-xl bg-[var(--card)] border border-[var(--border)] card-hover-soft h-full flex flex-col">
        <div className="relative overflow-hidden flex-shrink-0 aspect-[2/3] w-full bg-[var(--muted)]">
          ...
          <div className="absolute top-2 left-2 z-10">
            <div className="bg-violet-500/80 backdrop-blur-md text-white px-2 py-1 rounded-md text-xs font-semibold shadow-lg border border-violet-300/35 flex items-center gap-1">
              <Gem className="w-3 h-3" />
              <span>Недооцененный</span>
            </div>
          </div>
          ...
          <div className="absolute bottom-2 left-2 z-10">
            <div className="bg-black/55 backdrop-blur-md px-2 py-0.5 rounded-md flex items-center gap-1 text-[11px] font-medium text-white border border-white/20">
        ...
        <div className="p-3 bg-[var(--card)] rounded-b-xl flex flex-col min-h-[5.5rem]">
          <h3
            className={`${isAdultContent && !isAgeVerified ? "blur-sm" : ""} font-semibold text-sm text-[var(--foreground)] line-clamp-2 leading-snug group-hover:text-violet-500 transition-colors duration-300 min-h-[2.5rem] flex-1`}
          >
            {data.title}
          </h3>
        </div>
