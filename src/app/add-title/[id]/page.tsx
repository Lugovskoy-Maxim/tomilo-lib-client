...
export default function TitleEditorPage() {
  ...
  // Загрузка данных тайтла если редактируем
  useEffect(() => {
    if (titleId) {
      setIsEditMode(true);
  ...
  const loadTitleData = async (id: string) => {
    setIsLoading(true);
    try {
      ...
      setFormData(mockTitleData);
      setChapters(mockChapters);
    } catch (error) {
      console.error("Ошибка загрузки тайтла:", error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const parseChapterFromUrl = async (url: string): Promise<Omit<Chapter, 'id'> | null> => {
    ...
    try {
      ...
      const mockData = {
        ...
        title: `Глава ${nextChapterNumber} из парсера`,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isEditMode) {
      ...
      router.push(`/browse/${formData.id}`);
    } else {
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <p className="text-[var(--muted-foreground)]">
            {isEditMode 
              ? "Обновите информацию о тайтле и управляйте главами" 
          ...
          <div className="bg-[var(--card)] rounded-xl border border-[var(--border)] p-6">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex gap-2">
                ...
                >
                  {isEditMode ? "Редактировать" : "Ручное добавление"}
                </button>
                <button
                  type="button"
            ...
            {activeTab === "manual" ? (
              <div className="space-y-4">
                ...
                >
                  ...
                  {isEditMode ? "Добавить новую главу" : "Добавить главу"}
                </button>
              </div>
            ) : (
              <div className="space-y-4">
                ...
                {chapters.length > 0 && (
                  <div className="mt-6">
                    <h4 className="font-medium text-[var(--foreground)] mb-3">
                      {isEditMode ? "Все главы (старые и новые):" : "Добавленные главы:"}
                    </h4>
